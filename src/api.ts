...
/** Metadata for a document uploaded against a contact (bytes live server-side). */
export interface ContactDocument {
  ...
  size: number;
  mime: string;
  tab: 'internal' | 'sent' | 'received' | string;
  created_at: string | null;
}
...
export function documentFileUrl(id: number, download = false): string {
  return `${API_BASE}/documents/${id}${download ? '/download' : ''}`;
}
...
export const api = {
  ...
  createDocument: (
    contactId: number,
    input: { name: string; size: number; mime: string; tab?: string; data: string }
  ) =>
    request<{ data: { id: number }; message: string }>(`/contacts/${contactId}/documents`, {
      method: 'POST',
      body: JSON.stringify(input),
    }),
