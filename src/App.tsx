...
const handleAddSmartList = async (list: Omit<SmartList, 'id' | 'members'>) => {
  ...
  const handleUpdateContact = async (data: NewContactData) => {
    ...
    try {
      ...
      setEditContact(null);
      await reload();
      showToast(`Contact "${data.name}" updated successfully`);
      logActivity({ type: 'contact', title: 'Contact edited', detail: data.name });
    } catch (err) {
    ...
    <StaffProvider>
      {route.name === 'settings' ? (
        ...
        <div className="flex h-screen w-full overflow-hidden select-none bg-slate-100">
                ...
                {loading && contacts.length === 0 ? (
                  ...
                  <>
                    ...
                    <ContactsTable
                      ...
                      canDelete={hasActionPermission('contacts', 'Contacts', 'delete')}
                      canEdit={hasActionPermission('contacts', 'Contacts', 'edit')}
                    />
                  </>
                )}
          ...
          <AddContactDrawer
            open={addContactOpen || !!editContact}
            onClose={() => { setAddContactOpen(false); setEditContact(null); }}
            onSave={editContact ? handleUpdateContact : handleAddContact}
            onNotify={showToast}
